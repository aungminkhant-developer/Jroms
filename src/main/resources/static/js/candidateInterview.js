...
//to show success alert box when interview schedule create
if(interviewCreateSuccess!=null){
  togglePopupSuccess("Success", interviewCreateSuccess);
}
...
if (dropdownButton.innerText.includes('Depart') || dropdownButton.innerText.includes('Team')) {
...
function handleCB(event) {
  ...
  if (checkbox.checked) {
    ...
    mySelectedItems =
      mySelectedItems.filter((item) => item !== label);
  }

  dropdownButton.innerText = mySelectedItems.length > 0
    ? mySelectedItems.join(', ') : 'Choose Interviewer...';
}
...
function closePopupOnClickOutside(event) {
  var createInterview = document.getElementById('createInterview');
  if (!popupDiv.contains(event.target) && event.target !== createInterview) {
    hidePopup();
  }
}

//to show pop up for interview
document.getElementById('createInterview').addEventListener('click', function (event) {
  event.preventDefault();
  //to check mail status
  $.ajax({
    url: '/mng/interview/checkMailStatus/' + candidate.id,
    type: 'GET',
    success: function (data) {
      if(candidate.curriculumVitae.status !== 'CONSIDERING'){
        togglePopupWarning('Warning !!', 'Candidate CV status is not considering !!')
      }
      if (data === true && candidate.curriculumVitae.status === 'CONSIDERING') {
        togglePopupWarning('Mail Warning !!', 'Previous interview mail has not sent yet !!')
      }
      if(candidate.curriculumVitae.status === 'CONSIDERING' && data===false) {
        //to check interview result
        $.ajax({
          url: '/mng/interview/checkInterviewResult/' + candidate.id,
          type: 'GET',
          success: function (data) {
            if (data === true) {
              togglePopupWarning('Mail Warning !!', 'Please check previous interview result !!')
            }
            else {
              contactForm.removeAttribute('th:action');
              contactForm.setAttribute('action', '/mng/interview/candidateInterview/createInterview')
              createBtn.value = 'Create';
              togglePopup();
            }
          },
          error: function () {
            console.error('Error in check interviewresult when create interview')
          }
        })
      }
    },
    error: function () {
      console.error('Error in check mail status when create interview')
    }
  })
...
$(function () {
  ...
  const table = $('table#candidateInterview').DataTable({
    ajax: {
      contentType: 'application/json',
      url: '/mng/rest/candidateInterview?candidateId=' + candidate.id,
      type: 'POST',
      data: function (d) {
    ...
    columns: [
      ...
      },
      {
        ...
        render: function (data, type, row) {
          ...
          if (mailStatus === 'NEXT_MAIL_SEND' || mailStatus === 'NOT_SEND' || !isAuthorized) {
            return '<td><input type="hidden" class="resultValue" value="' + interviewId + '">' +
              '<p style="text-align:center;margin-top:15px" >' + data + '</p></td>'
          }
          else {
            return ' <td >' +
              '<input type="hidden" class="resultValue" value="' + interviewId + '">' +

              '<select style="color:#696969;width:200px;height:50px;" id="mySelect" data-mailstatus="' + mailStatus + '"' +
              ' data-show-content="true" class="form-select fa"' +
              'aria-label="Default select example"' +
              '><option value="NEXT_INTERVIEW"' + (data === 'NEXT_INTERVIEW' ? ' selected' : '') + '><span' +
              '>' + (data === 'NEXT_INTERVIEW' ? '&#xf058;' : '') + '</span>' +
              ' Next Interview</option>' +
              '<option value="PASSED"' + (data === 'PASSED' ? ' selected' : '') + '><span' +
              '>' + (data === 'PASSED' ? '&#xf058;' : '') + '</span>' +
              'Passed</option>' +
              '<option value="FAILED"' + (data === 'FAILED' ? ' selected' : '') + '><span' +
              '>' + (data === 'FAILED' ? '&#xf058;' : '') + '</span>' +
              'Failed</option>' +
              '<option value="PENDING"' + (data === 'PENDING' ? ' selected' : '') + '><span' +
              '>' + (data === 'PENDING' ? '&#xf058;' : '') + '</span>' +
              'Pending</option>' +
              '<option value="CANCEL"' + (data === 'CANCEL' ? ' selected' : '') + '><span' +
              '>' + (data === 'CANCEL' ? '&#xf058;' : '') + '</span>' +
              'Cancel</option>' +
              '</select>' +
        ...
        createdCell: function (td, cellData, rowData, row, col) {
          $(td).attr('id', 'resultSelect'); $(td).css({
            'max-width': '200px'
          });
        }, className: "align-middle"
      },
      ...
      },
      {
        ...
        render: function (data, type, row) {
          ...
          return '<a type="button" class="btn btn-outline-primary send-btn " data-candidatename="' + name + '" ' +
            ' style="margin-left:20px;"   data-email="' + candidateEmail + '" data-date1="' + date1 + '" ' +
            ' data-time1="' + time1 + '" data-time2="' + time2 + '"  data-interviewid="' + data + '"' +
            ' data-type="' + typeName + '" data-mailstatus="' + mailStatus + '" ' +
            ' data-value="' + typeValue + '"' +
            ' id="sendEamilBtn"  role="button">' +
            '<i class="fa-solid fa-paper-plane"></i></a>' +
            '<span class="position-absolute translate-middle p-1 ' +
            (mailStatus === 'NOT_SEND' ? 'text-danger fa-solid fa-circle-info' : 'text-success fa-solid fa-circle-check') + '">' +
      ...
      },
      {
        ...
        render: function (data, type, row) {
          var mailStatus = row.mailStatus;
          var interviewId = row.id;
          return '<div class="row w-100"><div class="col-md-5"><i class="fa-solid fa-pen-to-square edit-btn" data-mailstatus="' + mailStatus + '"></i>' +
            '<input type="hidden" value="' + data + '" class="eidtInterviewId"></div>' +
            '<div class="col-md-5"><i class="fa-solid fa-eye see-detail-interview" data-id="' + interviewId + '" ' +
            '"></i></div></div>';
        },
        createdCell: function(td){
          $(td).css({
  ...
  table.on('draw', function () {
      ...
      element.lastElementChild.addEventListener('change', function (e) {
        ...
        for (var i = 0; i < selectBox.options.length; i++) {
          if (i !== selectBox.selectedIndex) {
            ...
            switch (i) {
              case 0: option.innerHTML = 'Next Interview'; break;
              case 1: option.innerHTML = 'Passed'; break;
              case 2: option.innerHTML = 'Failed'; break;
              case 3: option.innerHTML = 'Pending'; break;
              case 4: option.innerHTML = 'Cancel'; break;
            }
          };
        };
        ...
        for (var j = 0; j < statusChange.length; j++) {
          ...
          if (selectedOption.value === 'PASSED' || selectedOption.value === 'CANCEL' || selectedOption.value === 'FAILED') {
            if (j === indexOfSelectElement) {
              statusValue.innerHTML = 'FINISHED';
              addStatus = 'FINISHED';
            };
          };
          if (selectedOption.value === 'NEXT_INTERVIEW' || selectedOption.value === 'PENDING') {
            if (j === indexOfSelectElement) {
    ...
    var candidateNameMail, startDateMail, startTimeMail, endTimeMail, typeNameMail, typeValueMail;
      ...
      el.onclick = (e) => {
        ...
        if (targetBtn) {
          ...
          if (mailStatus === 'NOT_SEND') {
            candidateIdForSendMail.value = candidate.id;
            ...
            redirectPage.value = 2;
            console.log('hihie22ihih2ih')
            //check when have any interviews
            if (candidateInterviewCount > 1) {
              //to check interview result by candidate Id
              $.ajax({
                url: '/mng/interview/checkIneterviewResultByMailStatus/' + candidate.id,
                type: 'GET',
                success: function (data) {
        ...
        fetch('/mng/mail/getEmailType', {
          ...
          .then(data => {
            var onlineMetting, location, locationValue;
              if (typeNameMail.includes('Online') || typeNameMail.includes('online') || typeNameMail.includes('ONLINE')) {
                onlineMetting = '<p>Join Zoom Meeting</p>' +
                  '<p contenteditable="false">' + typeValueMail + '</p>';
                location = 'URL';
              ...
              var insertContent = '<p>&nbsp;</p>' +
                '<p>Interview Schedule</p>' +
                '<p contenteditable="false">Date&nbsp; &nbsp; :&nbsp; ' + startDateMail + '</p>' +
                '<p contenteditable="false">Time&nbsp; &nbsp; :&nbsp; &nbsp;' + startTimeMail + '- ' + endTimeMail + '</p>' +
                '<p contenteditable="false">Type&nbsp; &nbsp; :&nbsp; &nbsp; ' + typeNameMail + '</p>' +
                '<p contenteditable="false">' + location + ': ' + locationValue + '</p>' +
                onlineMetting;
        ...
        if (targetBtn) {
          ...
          fetch('/mng/interview/editInterview', {
            ...
            .then(data => {
              ...
              $('#interviewModal span').eq(0).text(candidate.name);
              $('#interviewModal span').eq(1).text(candidate.jobDetail.jobTitle.name + (candidate.jobDetail.jobLevel != null ? '(' + candidate.jobDetail.jobLevel.name + ')' : ''));
              ...
              $('#interviewModal span').eq(5).text(data.startTimeFormat + '-' + data.endTimeFormat);
              $('#interviewModal span').eq(6).text(data.interviewType);
              $('#interviewModal p strong').eq(7).text(data.interviewType.includes('online') || data.interviewType.includes('Online') ? 'URL:' : 'Location:')
              $('#interviewModal span').eq(7).text(data.interviewType.includes('online') || data.interviewType.includes('Online') ? data.typeValue.substring(0, 30) + '...' : data.typeValue);
    ...
    document.querySelectorAll('.edit-btn').forEach((element, index) => {
      element.addEventListener('click', function (e) {
        var mailStatus = e.target.dataset.mailstatus;
        if (mailStatus === 'NOT_SEND') {
          var editInterviewId = document.querySelectorAll('.eidtInterviewId')[index];
          ...
          fetch('/mng/interview/editInterview', {
            ...
            .then(data => {
              ...
              for (var i = 0; i < selectBox.options.length; i++) {
                ...
                else selectBox.options[i].removeAttribute('selected', 'selected');

              };
              //to be selected interviewer
              if (data.interviewerList[0] != null && data.interviewerList[1] != null) {
                ...
                if (data.interviewerList[0] != null && data.interviewerList[0].includes('DEPART')) {
                  $('#dropdown-menu input').eq(0).prop('checked', true);
                  mySelectedItems.push('Department Head'),
                    dropdownButton.innerText = 'Department Head';
                }
                if (data.interviewerList[0] != null && data.interviewerList[0].includes('TEAM')) {
                  $('#dropdown-menu input').eq(1).prop('checked', true);
                  mySelectedItems.push('Team Leader')
                  dropdownButton.innerText = 'Team Leader';
                }
              ...
              contactForm.setAttribute('action', '/mng/interview/updateInterview')
          ...
          togglePopupWarning('Warning !!', 'This interview schedule has been sent ')
        }

      })
    });//end tag of edit interviews

  });//end tag of table on draw
