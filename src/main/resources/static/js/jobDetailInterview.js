...
function closePopUp() {
    ...
    //to remove selected interview stage
...
$(function () {
    ...
    $("#finalStage").on('change', function () {
        ...
        if (stage == 1) {
            dynamicURL = '/mng/rest/jobDetailInterview/finalStage?jobDetailId=' + jobDetail.id;
        } else {
            dynamicURL = '/mng/rest/jobDetailInterview?jobDetailId=' + jobDetail.id;
        }
    ...
    const table = $('table#interview').DataTable({

        ajax: {
            contentType: 'application/json',
            url: '/mng/rest/jobDetailInterview?jobDetailId=' + jobDetail.id,
            type: 'POST',
            data: function (d) {
        ...
        columns: [
            ...
            { data: 'candidate.name', className: "align-middle" ,createdCell: function (td, cellData, rowData, row, col) {
            ...
            { data: 'interviewSchedule.interviewType.name',createdCell: function (td, cellData, rowData, row, col) {
                $(td).attr('style','width:10%')
            }, className: "align-middle" },
            { data: 'interviewStage.name', createdCell: function (td, cellData, rowData, row, col) {
                $(td).attr('style','width:13%')
            },
            className: "align-middle" },
            ...
            },
            {
                ...
                render: function (data, type, row) {
                    ...
                    return '<a type="button" class="btn btn-outline-primary send-btn "  data-candidatename="' + name + '" ' +
                        ...
                        ' data-value="' + typeValue + '" data-candidateid="' + candidateId + '"' +
            ...
            },
            {
                ...
                render: function (data, type, row) {
                    ...
                    var interviewId = row.id;
                    return '<div class="row w-100"><div class="col-md-3"><i class="fa-solid fa-pen-to-square edit-btn" ' +
                        'data-candidateid="' + candidateId2 + '" data-mailstatus="' + mailStatus + '"></i>' +
                        '<input type="hidden" value="' + data + '" class="eidtInterviewId"></div>' +
                        '<div class="col-md-3"><i class="fa-solid fa-eye see-detail-interview" data-id="' + interviewId + '" ' +
                        '"></i></div>' +
                        '<div class="col-md-3"><a href="/mng/interview/candidateInterview?candidateId=' + candidateId2 + '"><i class="fa-solid fa-angles-right" ' +
                        '"></i></a></div></div>';
                },
                createdCell: function (td) {
            ...
            el.onclick = (e) => {
                ...
                if (targetBtn) {
                    ...
                    if (mailStatus === 'NOT_SEND') {
                        ...
                        redirectPage.value = 0;
                        //get candidate count to check confirmBox
                        $.ajax({
                            url: '/mng/interview/candidateCount/' + targetBtn.dataset.candidateid,
                            type: 'GET',
                            success: function (data) {
                                if (data > 1) {
                                    //to check interview result by candidate Id
                                    $.ajax({
                                        url: '/mng/interview/checkIneterviewResultByMailStatus/' + targetBtn.dataset.candidateid,
                                        type: 'GET',
                                        success: function (data) {
                            ...
                            error: function () {
                                console.error('Error in getting candidate count for confirmation box')
                            }
                        })
                    }
                    else {
                ...
                fetch('/mng/mail/getEmailType', {
                    ...
                    .then(data => {
                        ...
                        if (typeNameMail.includes('Online') || typeNameMail.includes('online') || typeNameMail.includes('ONLINE')) {
                            onlineMetting = '<p><strong>Join Zoom Meeting</strong></p>' +
                                '<p contenteditable="false"><strong>' + typeValueMail + '</strong></p>';
                            location = 'URL';
                ...
                if (mailStatus === 'NOT_SEND') {
                    ...
                    fetch('/mng/interview/editInterview', {
                        ...
                        .then(data => {
                            ...
                            jobDetailId.value = jobDetail.id;
                ...
                if (targetBtn) {
                    ...
                    fetch('/mng/interview/editInterview', {
                        ...
                        .then(data => {
                            ...
                            $('#interviewModal span').eq(1).text(jobDetail.jobTitle.name + (jobDetail.jobLevel != null ? '(' + jobDetail.jobLevel.name + ')' : ''));
                            ...
                            $('#interviewModal span').eq(7).text(data.interviewType.includes('online') || data.interviewType.includes('Online') ? data.typeValue.substring(0, 20) + '...' : data.typeValue);
